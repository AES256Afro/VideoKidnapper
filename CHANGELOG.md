...
- **Multi-line captions sat lower in the preview than in the export.** The preview spaced lines tighter than ffmpeg does, by up to 20 pixels on a four-line caption. It now uses ffmpeg's line spacing and matches to the pixel.
...
- **Auto-track (⚡).** Instead of keyframing by hand, drop the caption on the thing you want to label and click **⚡ Auto-track from here** — OpenCV follows that patch of video from the playhead to the end of the trim range and fills in the motion path for you. Optional feature (`pip install "videokidnapper[track]"`, or the button tells you how); the manual path works without it. Tracks best on a textured, distinct subject (a face or hand), and if it loses the target the path simply ends at the last confident point — scrub to check and drag any keyframe to fix it. The dense samples are simplified (Ramer-Douglas-Peucker) so straight runs collapse to a couple of points and the compiled drawtext expression stays small.
